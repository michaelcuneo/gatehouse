hoistPattern:
  - '*'
hoistedDependencies:
  '@aws-sdk/client-cloudwatch-logs@3.1138.0':
    '@aws-sdk/client-cloudwatch-logs': private
  '@aws-sdk/client-cloudwatch@3.1138.0':
    '@aws-sdk/client-cloudwatch': private
  '@aws-sdk/client-dynamodb@3.1138.0':
    '@aws-sdk/client-dynamodb': private
  '@aws-sdk/client-lambda@3.1138.0':
    '@aws-sdk/client-lambda': private
  '@aws-sdk/client-sts@3.1138.0':
    '@aws-sdk/client-sts': private
  '@aws-sdk/core@3.978.1':
    '@aws-sdk/core': private
  '@aws-sdk/credential-provider-env@3.972.72':
    '@aws-sdk/credential-provider-env': private
  '@aws-sdk/credential-provider-http@3.972.74':
    '@aws-sdk/credential-provider-http': private
  '@aws-sdk/credential-provider-ini@3.973.17':
    '@aws-sdk/credential-provider-ini': private
  '@aws-sdk/credential-provider-login@3.972.79':
    '@aws-sdk/credential-provider-login': private
  '@aws-sdk/credential-provider-node@3.972.84':
    '@aws-sdk/credential-provider-node': private
  '@aws-sdk/credential-provider-process@3.972.72':
    '@aws-sdk/credential-provider-process': private
  '@aws-sdk/credential-provider-sso@3.973.16':
    '@aws-sdk/credential-provider-sso': private
  '@aws-sdk/credential-provider-web-identity@3.972.78':
    '@aws-sdk/credential-provider-web-identity': private
  '@aws-sdk/dynamodb-codec@3.973.46':
    '@aws-sdk/dynamodb-codec': private
  '@aws-sdk/endpoint-cache@3.972.11':
    '@aws-sdk/endpoint-cache': private
  '@aws-sdk/middleware-endpoint-discovery@3.972.31':
    '@aws-sdk/middleware-endpoint-discovery': private
  '@aws-sdk/nested-clients@3.997.46':
    '@aws-sdk/nested-clients': private
  '@aws-sdk/signature-v4-multi-region@3.996.47':
    '@aws-sdk/signature-v4-multi-region': private
  '@aws-sdk/token-providers@3.1138.0':
    '@aws-sdk/token-providers': private
  '@aws-sdk/types@3.974.6':
    '@aws-sdk/types': private
  '@aws-sdk/xml-builder@3.972.41':
    '@aws-sdk/xml-builder': private
  '@aws/lambda-invoke-store@0.3.0':
    '@aws/lambda-invoke-store': private
  '@eslint-community/eslint-utils@4.9.1(eslint@10.4.0)':
    '@eslint-community/eslint-utils': public
  '@eslint-community/regexpp@4.12.2':
    '@eslint-community/regexpp': public
  '@eslint/compat@2.1.0(eslint@10.4.0)':
    '@eslint/compat': public
  '@eslint/config-array@0.23.5':
    '@eslint/config-array': public
  '@eslint/config-helpers@0.6.0':
    '@eslint/config-helpers': public
  '@eslint/core@1.2.1':
    '@eslint/core': public
  '@eslint/js@10.0.1(eslint@10.4.0)':
    '@eslint/js': public
  '@eslint/object-schema@3.0.5':
    '@eslint/object-schema': public
  '@eslint/plugin-kit@0.7.1':
    '@eslint/plugin-kit': public
  '@humanfs/core@0.19.2':
    '@humanfs/core': private
  '@humanfs/node@0.16.8':
    '@humanfs/node': private
  '@humanfs/types@0.15.0':
    '@humanfs/types': private
  '@humanwhocodes/module-importer@1.0.1':
    '@humanwhocodes/module-importer': private
  '@humanwhocodes/retry@0.4.3':
    '@humanwhocodes/retry': private
  '@jridgewell/gen-mapping@0.3.13':
    '@jridgewell/gen-mapping': private
  '@jridgewell/remapping@2.3.5':
    '@jridgewell/remapping': private
  '@jridgewell/resolve-uri@3.1.2':
    '@jridgewell/resolve-uri': private
  '@jridgewell/sourcemap-codec@1.5.5':
    '@jridgewell/sourcemap-codec': private
  '@jridgewell/trace-mapping@0.3.31':
    '@jridgewell/trace-mapping': private
  '@oxc-project/types@0.132.0':
    '@oxc-project/types': private
  '@polka/url@1.0.0-next.29':
    '@polka/url': private
  '@rolldown/binding-android-arm64@1.0.2':
    '@rolldown/binding-android-arm64': private
  '@rolldown/binding-darwin-arm64@1.0.2':
    '@rolldown/binding-darwin-arm64': private
  '@rolldown/binding-darwin-x64@1.0.2':
    '@rolldown/binding-darwin-x64': private
  '@rolldown/binding-freebsd-x64@1.0.2':
    '@rolldown/binding-freebsd-x64': private
  '@rolldown/binding-linux-arm-gnueabihf@1.0.2':
    '@rolldown/binding-linux-arm-gnueabihf': private
  '@rolldown/binding-linux-arm64-gnu@1.0.2':
    '@rolldown/binding-linux-arm64-gnu': private
  '@rolldown/binding-linux-arm64-musl@1.0.2':
    '@rolldown/binding-linux-arm64-musl': private
  '@rolldown/binding-linux-ppc64-gnu@1.0.2':
    '@rolldown/binding-linux-ppc64-gnu': private
  '@rolldown/binding-linux-s390x-gnu@1.0.2':
    '@rolldown/binding-linux-s390x-gnu': private
  '@rolldown/binding-linux-x64-gnu@1.0.2':
    '@rolldown/binding-linux-x64-gnu': private
  '@rolldown/binding-linux-x64-musl@1.0.2':
    '@rolldown/binding-linux-x64-musl': private
  '@rolldown/binding-openharmony-arm64@1.0.2':
    '@rolldown/binding-openharmony-arm64': private
  '@rolldown/binding-wasm32-wasi@1.0.2':
    '@rolldown/binding-wasm32-wasi': private
  '@rolldown/binding-win32-arm64-msvc@1.0.2':
    '@rolldown/binding-win32-arm64-msvc': private
  '@rolldown/binding-win32-x64-msvc@1.0.2':
    '@rolldown/binding-win32-x64-msvc': private
  '@rolldown/pluginutils@1.0.1':
    '@rolldown/pluginutils': private
  '@smithy/core@3.35.0':
    '@smithy/core': private
  '@smithy/credential-provider-imds@4.5.2':
    '@smithy/credential-provider-imds': private
  '@smithy/fetch-http-handler@5.8.0':
    '@smithy/fetch-http-handler': private
  '@smithy/middleware-compression@4.6.2':
    '@smithy/middleware-compression': private
  '@smithy/node-http-handler@4.12.1':
    '@smithy/node-http-handler': private
  '@smithy/signature-v4@5.7.3':
    '@smithy/signature-v4': private
  '@smithy/types@4.19.0':
    '@smithy/types': private
  '@standard-schema/spec@1.1.0':
    '@standard-schema/spec': private
  '@sveltejs/acorn-typescript@1.0.10(acorn@8.16.0)':
    '@sveltejs/acorn-typescript': private
  '@sveltejs/adapter-auto@7.0.1(@sveltejs/kit@2.61.1(@sveltejs/vite-plugin-svelte@7.1.2(svelte@5.55.10(@typescript-eslint/types@8.60.0))(vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3)))(svelte@5.55.10(@typescript-eslint/types@8.60.0))(typescript@6.0.3)(vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3)))':
    '@sveltejs/adapter-auto': private
  '@sveltejs/kit@2.61.1(@sveltejs/vite-plugin-svelte@7.1.2(svelte@5.55.10(@typescript-eslint/types@8.60.0))(vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3)))(svelte@5.55.10(@typescript-eslint/types@8.60.0))(typescript@6.0.3)(vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3))':
    '@sveltejs/kit': private
  '@sveltejs/vite-plugin-svelte@7.1.2(svelte@5.55.10(@typescript-eslint/types@8.60.0))(vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3))':
    '@sveltejs/vite-plugin-svelte': private
  '@turbo/darwin-64@2.9.15':
    '@turbo/darwin-64': private
  '@turbo/darwin-arm64@2.9.15':
    '@turbo/darwin-arm64': private
  '@turbo/linux-64@2.9.15':
    '@turbo/linux-64': private
  '@turbo/linux-arm64@2.9.15':
    '@turbo/linux-arm64': private
  '@turbo/windows-64@2.9.15':
    '@turbo/windows-64': private
  '@turbo/windows-arm64@2.9.15':
    '@turbo/windows-arm64': private
  '@types/better-sqlite3@7.6.13':
    '@types/better-sqlite3': private
  '@types/cookie@0.6.0':
    '@types/cookie': private
  '@types/esrecurse@4.3.1':
    '@types/esrecurse': private
  '@types/estree@1.0.9':
    '@types/estree': private
  '@types/json-schema@7.0.15':
    '@types/json-schema': private
  '@types/trusted-types@2.0.7':
    '@types/trusted-types': private
  '@typescript-eslint/eslint-plugin@8.60.0(@typescript-eslint/parser@8.60.0(eslint@10.4.0)(typescript@6.0.3))(eslint@10.4.0)(typescript@6.0.3)':
    '@typescript-eslint/eslint-plugin': public
  '@typescript-eslint/parser@8.60.0(eslint@10.4.0)(typescript@6.0.3)':
    '@typescript-eslint/parser': public
  '@typescript-eslint/project-service@8.60.0(typescript@6.0.3)':
    '@typescript-eslint/project-service': public
  '@typescript-eslint/scope-manager@8.60.0':
    '@typescript-eslint/scope-manager': public
  '@typescript-eslint/tsconfig-utils@8.60.0(typescript@6.0.3)':
    '@typescript-eslint/tsconfig-utils': public
  '@typescript-eslint/type-utils@8.60.0(eslint@10.4.0)(typescript@6.0.3)':
    '@typescript-eslint/type-utils': public
  '@typescript-eslint/types@8.60.0':
    '@typescript-eslint/types': public
  '@typescript-eslint/typescript-estree@8.60.0(typescript@6.0.3)':
    '@typescript-eslint/typescript-estree': public
  '@typescript-eslint/utils@8.60.0(eslint@10.4.0)(typescript@6.0.3)':
    '@typescript-eslint/utils': public
  '@typescript-eslint/visitor-keys@8.60.0':
    '@typescript-eslint/visitor-keys': public
  acorn-jsx@5.3.2(acorn@8.16.0):
    acorn-jsx: private
  acorn@8.16.0:
    acorn: private
  ajv@6.15.0:
    ajv: private
  apps/web:
    '@gatehouse/web': private
  aria-query@5.3.1:
    aria-query: private
  axobject-query@4.1.0:
    axobject-query: private
  balanced-match@4.0.4:
    balanced-match: private
  base64-js@1.5.1:
    base64-js: private
  better-sqlite3@12.10.0:
    better-sqlite3: private
  bindings@1.5.0:
    bindings: private
  bl@4.1.0:
    bl: private
  bowser@2.14.1:
    bowser: private
  brace-expansion@5.0.6:
    brace-expansion: private
  buffer@5.7.1:
    buffer: private
  chokidar@4.0.3:
    chokidar: private
  chownr@1.1.4:
    chownr: private
  clsx@2.1.1:
    clsx: private
  cookie@0.6.0:
    cookie: private
  cross-spawn@7.0.6:
    cross-spawn: private
  cssesc@3.0.0:
    cssesc: private
  debug@4.4.3:
    debug: private
  decompress-response@6.0.0:
    decompress-response: private
  deep-extend@0.6.0:
    deep-extend: private
  deep-is@0.1.4:
    deep-is: private
  deepmerge@4.3.1:
    deepmerge: private
  detect-libc@2.1.2:
    detect-libc: private
  devalue@5.8.1:
    devalue: private
  end-of-stream@1.4.5:
    end-of-stream: private
  escape-string-regexp@4.0.0:
    escape-string-regexp: private
  eslint-config-prettier@10.1.8(eslint@10.4.0):
    eslint-config-prettier: public
  eslint-plugin-svelte@3.18.0(eslint@10.4.0)(svelte@5.55.10(@typescript-eslint/types@8.60.0)):
    eslint-plugin-svelte: public
  eslint-scope@9.1.2:
    eslint-scope: public
  eslint-visitor-keys@5.0.1:
    eslint-visitor-keys: public
  eslint@10.4.0:
    eslint: public
  esm-env@1.2.2:
    esm-env: private
  espree@11.2.0:
    espree: private
  esquery@1.7.0:
    esquery: private
  esrap@2.2.9(@typescript-eslint/types@8.60.0):
    esrap: private
  esrecurse@4.3.0:
    esrecurse: private
  estraverse@5.3.0:
    estraverse: private
  esutils@2.0.3:
    esutils: private
  expand-template@2.0.3:
    expand-template: private
  fast-deep-equal@3.1.3:
    fast-deep-equal: private
  fast-json-stable-stringify@2.1.0:
    fast-json-stable-stringify: private
  fast-levenshtein@2.0.6:
    fast-levenshtein: private
  fdir@6.5.0(picomatch@4.0.4):
    fdir: private
  fflate@0.8.3:
    fflate: private
  file-entry-cache@8.0.0:
    file-entry-cache: private
  file-uri-to-path@1.0.0:
    file-uri-to-path: private
  find-up@5.0.0:
    find-up: private
  flat-cache@4.0.1:
    flat-cache: private
  flatted@3.4.2:
    flatted: private
  fs-constants@1.0.0:
    fs-constants: private
  fsevents@2.3.3:
    fsevents: private
  github-from-package@0.0.0:
    github-from-package: private
  glob-parent@6.0.2:
    glob-parent: private
  globals@17.6.0:
    globals: private
  ieee754@1.2.1:
    ieee754: private
  ignore@5.3.2:
    ignore: private
  imurmurhash@0.1.4:
    imurmurhash: private
  inherits@2.0.4:
    inherits: private
  ini@1.3.8:
    ini: private
  is-extglob@2.1.1:
    is-extglob: private
  is-glob@4.0.3:
    is-glob: private
  is-reference@3.0.3:
    is-reference: private
  isexe@2.0.0:
    isexe: private
  json-buffer@3.0.1:
    json-buffer: private
  json-schema-traverse@0.4.1:
    json-schema-traverse: private
  json-stable-stringify-without-jsonify@1.0.1:
    json-stable-stringify-without-jsonify: private
  keyv@4.5.4:
    keyv: private
  kleur@4.1.5:
    kleur: private
  known-css-properties@0.37.0:
    known-css-properties: private
  levn@0.4.1:
    levn: private
  lightningcss-android-arm64@1.32.0:
    lightningcss-android-arm64: private
  lightningcss-darwin-arm64@1.32.0:
    lightningcss-darwin-arm64: private
  lightningcss-darwin-x64@1.32.0:
    lightningcss-darwin-x64: private
  lightningcss-freebsd-x64@1.32.0:
    lightningcss-freebsd-x64: private
  lightningcss-linux-arm-gnueabihf@1.32.0:
    lightningcss-linux-arm-gnueabihf: private
  lightningcss-linux-arm64-gnu@1.32.0:
    lightningcss-linux-arm64-gnu: private
  lightningcss-linux-arm64-musl@1.32.0:
    lightningcss-linux-arm64-musl: private
  lightningcss-linux-x64-gnu@1.32.0:
    lightningcss-linux-x64-gnu: private
  lightningcss-linux-x64-musl@1.32.0:
    lightningcss-linux-x64-musl: private
  lightningcss-win32-arm64-msvc@1.32.0:
    lightningcss-win32-arm64-msvc: private
  lightningcss-win32-x64-msvc@1.32.0:
    lightningcss-win32-x64-msvc: private
  lightningcss@1.32.0:
    lightningcss: private
  lilconfig@2.1.0:
    lilconfig: private
  locate-character@3.0.0:
    locate-character: private
  locate-path@6.0.0:
    locate-path: private
  magic-string@0.30.21:
    magic-string: private
  mimic-response@3.1.0:
    mimic-response: private
  minimatch@10.2.5:
    minimatch: private
  minimist@1.2.8:
    minimist: private
  mkdirp-classic@0.5.3:
    mkdirp-classic: private
  mnemonist@0.38.3:
    mnemonist: private
  mri@1.2.0:
    mri: private
  mrmime@2.0.1:
    mrmime: private
  ms@2.1.3:
    ms: private
  nanoid@3.3.12:
    nanoid: private
  napi-build-utils@2.0.0:
    napi-build-utils: private
  natural-compare@1.4.0:
    natural-compare: private
  node-abi@3.92.0:
    node-abi: private
  obliterator@1.6.1:
    obliterator: private
  obug@2.1.1:
    obug: private
  once@1.4.0:
    once: private
  optionator@0.9.4:
    optionator: private
  p-limit@3.1.0:
    p-limit: private
  p-locate@5.0.0:
    p-locate: private
  packages/aws:
    '@gatehouse/aws': private
  packages/core:
    '@gatehouse/core': private
  packages/db:
    '@gatehouse/db': private
  packages/observability:
    '@gatehouse/observability': private
  packages/providers:
    '@gatehouse/providers': private
  packages/reconciliation:
    '@gatehouse/reconciliation': private
  packages/resources:
    '@gatehouse/resources': private
  packages/runtime:
    '@gatehouse/runtime': private
  packages/types:
    '@gatehouse/types': private
  path-exists@4.0.0:
    path-exists: private
  path-key@3.1.1:
    path-key: private
  picocolors@1.1.1:
    picocolors: private
  picomatch@4.0.4:
    picomatch: private
  postcss-load-config@3.1.4(postcss@8.5.15):
    postcss-load-config: private
  postcss-safe-parser@7.0.1(postcss@8.5.15):
    postcss-safe-parser: private
  postcss-scss@4.0.9(postcss@8.5.15):
    postcss-scss: private
  postcss-selector-parser@7.1.1:
    postcss-selector-parser: private
  postcss@8.5.15:
    postcss: private
  prebuild-install@7.1.3:
    prebuild-install: private
  prelude-ls@1.2.1:
    prelude-ls: private
  prettier-plugin-svelte@3.5.2(prettier@3.8.3)(svelte@5.55.10(@typescript-eslint/types@8.60.0)):
    prettier-plugin-svelte: public
  pump@3.0.4:
    pump: private
  punycode@2.3.1:
    punycode: private
  rc@1.2.8:
    rc: private
  readable-stream@3.6.2:
    readable-stream: private
  readdirp@4.1.2:
    readdirp: private
  rolldown@1.0.2:
    rolldown: private
  sade@1.8.1:
    sade: private
  safe-buffer@5.2.1:
    safe-buffer: private
  semver@7.8.1:
    semver: private
  set-cookie-parser@3.1.0:
    set-cookie-parser: private
  shebang-command@2.0.0:
    shebang-command: private
  shebang-regex@3.0.0:
    shebang-regex: private
  simple-concat@1.0.1:
    simple-concat: private
  simple-get@4.0.1:
    simple-get: private
  sirv@3.0.2:
    sirv: private
  source-map-js@1.2.1:
    source-map-js: private
  string_decoder@1.3.0:
    string_decoder: private
  strip-json-comments@2.0.1:
    strip-json-comments: private
  svelte-check@4.4.8(picomatch@4.0.4)(svelte@5.55.10(@typescript-eslint/types@8.60.0))(typescript@6.0.3):
    svelte-check: private
  svelte-eslint-parser@1.6.1(svelte@5.55.10(@typescript-eslint/types@8.60.0)):
    svelte-eslint-parser: public
  svelte@5.55.10(@typescript-eslint/types@8.60.0):
    svelte: private
  tar-fs@2.1.4:
    tar-fs: private
  tar-stream@2.2.0:
    tar-stream: private
  tinyglobby@0.2.16:
    tinyglobby: private
  totalist@3.0.1:
    totalist: private
  ts-api-utils@2.5.0(typescript@6.0.3):
    ts-api-utils: private
  tslib@2.8.1:
    tslib: private
  tunnel-agent@0.6.0:
    tunnel-agent: private
  type-check@0.4.0:
    type-check: private
  typescript-eslint@8.60.0(eslint@10.4.0)(typescript@6.0.3):
    typescript-eslint: public
  undici-types@6.21.0:
    undici-types: private
  uri-js@4.4.1:
    uri-js: private
  util-deprecate@1.0.2:
    util-deprecate: private
  vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3):
    vite: private
  vitefu@1.1.3(vite@8.0.14(@types/node@22.19.19)(yaml@1.10.3)):
    vitefu: private
  which@2.0.2:
    which: private
  word-wrap@1.2.5:
    word-wrap: private
  wrappy@1.0.2:
    wrappy: private
  yaml@1.10.3:
    yaml: private
  yocto-queue@0.1.0:
    yocto-queue: private
  zimmerframe@1.1.4:
    zimmerframe: private
included:
  dependencies: true
  devDependencies: true
  optionalDependencies: true
injectedDeps: {}
layoutVersion: 5
nodeLinker: isolated
packageManager: pnpm@9.0.0
pendingBuilds: []
prunedAt: Wed, 23 Sep 2026 00:32:45 GMT
publicHoistPattern:
  - '*eslint*'
  - '*prettier*'
registries:
  default: https://registry.npmjs.org/
skipped:
  - '@emnapi/core@1.10.0'
  - '@emnapi/runtime@1.10.0'
  - '@emnapi/wasi-threads@1.2.1'
  - '@napi-rs/wasm-runtime@1.1.4(@emnapi/core@1.10.0)(@emnapi/runtime@1.10.0)'
  - '@rolldown/binding-android-arm64@1.0.2'
  - '@rolldown/binding-darwin-arm64@1.0.2'
  - '@rolldown/binding-darwin-x64@1.0.2'
  - '@rolldown/binding-freebsd-x64@1.0.2'
  - '@rolldown/binding-linux-arm-gnueabihf@1.0.2'
  - '@rolldown/binding-linux-arm64-gnu@1.0.2'
  - '@rolldown/binding-linux-arm64-musl@1.0.2'
  - '@rolldown/binding-linux-ppc64-gnu@1.0.2'
  - '@rolldown/binding-linux-s390x-gnu@1.0.2'
  - '@rolldown/binding-openharmony-arm64@1.0.2'
  - '@rolldown/binding-wasm32-wasi@1.0.2'
  - '@rolldown/binding-win32-arm64-msvc@1.0.2'
  - '@rolldown/binding-win32-x64-msvc@1.0.2'
  - '@turbo/darwin-64@2.9.15'
  - '@turbo/darwin-arm64@2.9.15'
  - '@turbo/linux-arm64@2.9.15'
  - '@turbo/windows-64@2.9.15'
  - '@turbo/windows-arm64@2.9.15'
  - '@tybys/wasm-util@0.10.2'
  - fsevents@2.3.3
  - lightningcss-android-arm64@1.32.0
  - lightningcss-darwin-arm64@1.32.0
  - lightningcss-darwin-x64@1.32.0
  - lightningcss-freebsd-x64@1.32.0
  - lightningcss-linux-arm-gnueabihf@1.32.0
  - lightningcss-linux-arm64-gnu@1.32.0
  - lightningcss-linux-arm64-musl@1.32.0
  - lightningcss-win32-arm64-msvc@1.32.0
  - lightningcss-win32-x64-msvc@1.32.0
storeDir: /home/michael/.local/share/pnpm/store/v3
virtualStoreDir: .pnpm
